...
export const makeRenderQueue = ({
  ...
  const processRender = async (jobId: string) => {
    ...
    jobs.set(jobId, { progress: 0, status: "in-progress", cancel, data: job.data });
    ...
    try {
      ...
      await renderMedia({
        ...
        onProgress: (progress) => {
          ...
          jobs.set(jobId, { progress: progress.progress, status: "in-progress", cancel, data: job.data });
        },
      ...
      console.error(error);
      jobs.set(jobId, { status: "failed", error: error as Error, data: job.data });
    }
  };
